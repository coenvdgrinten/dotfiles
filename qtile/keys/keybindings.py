...
from config_keybindings import *  # noqa: F403
...
class Keybindings:
    ...
    def create_kill_keys(self):
        ...
        all_minus_current = Key(
            modifier,
            KILL_ALL_MINUS_CURRENT,
            lazy.function(kill_all_windows_minus_current),
        )
        all_ = Key(modifier, KILL_ALL, lazy.function(kill_all_windows))
        current = Key([KILL_KEY], KILL_CURRENT, lazy.window.kill())
    ...
    def create_groups_keys(self):
        ...
        swap_next = Key(swap_modifier, NEXT_GROUP, lazy.function(window_to_next_group))
        swap_prev = Key(swap_modifier, PREV_GROUP, lazy.function(window_to_prev_group))
